...
import com.cars.app.config.KafkaProperties;
...
class CarsappKafkaResourceIT {
    ...
    void setup() {
        ...
        CarsappKafkaResource kafkaResource = new CarsappKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/carsapp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/carsapp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
